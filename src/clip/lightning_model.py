...
class CLIPLightning(L.LightningModule):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.real_len, self.fake_len = len(
            real_prompts) - 1, len(fake_prompts) - 1
        ...
        self.auc_fn = AUROC(task='multiclass',
                            average='weighted', num_classes=2)
        self.prec_fn = Precision(
            task="multiclass", average='weighted', num_classes=2)
    ...
    def configure_optimizers(self) -> Any:
        ...
        ) if self.only_vision else self.model.parameters()
    ...
    def training_step(self, batch, batch_idx) -> torch.tensor:
        ...
        img_logits = torch.stack(
            (real_logits_per_image, fake_logits_per_image), dim=1)
        txt_logits = torch.stack(
            (real_logits_per_text, fake_logits_per_text), dim=1)
        txt_logits = rearrange(txt_logits, "r f b -> b r f")
        # img_logits and txt_logits shape: (batch, 2, c, h, w)  -> 2: 1 real and 1 fake

        all_loss_img = [cross_entropy(img_couple, self.labels)
                        for img_couple in img_logits]
        all_loss_txt = [cross_entropy(txt_couple, self.labels)
                        for txt_couple in txt_logits]

        clip_loss = torch.stack([(loss_img + loss_txt) / 2 for loss_img,
                                loss_txt in zip(all_loss_img, all_loss_txt)]).mean()

        if self.contrastive_margin is not None:
            contrastive_loss = self.contrastive_loss(
                real_feats_img, fake_feats_img, self.contrastive_target).mean()
    ...
    def test_step(self, batch, batch_idx) -> Any:
        ...
        _, _, logits_per_image, logits_per_text = self.model(
            x, self.prompt_feats)

        if self.topk is not None:
            topk_vals, topk_idx = torch.topk(
                logits_per_image, k=self.topk, dim=-1)
            real_idxes = topk_idx < self.prompt_split
            fake_idxes = topk_idx >= self.prompt_split

            real_logs = (topk_vals*real_idxes).sum(dim=-1) / \
                real_idxes.sum(dim=-1)
            fake_logs = (topk_vals*fake_idxes).sum(dim=-1) / \
                fake_idxes.sum(dim=-1)
    ...
    def get_labels(self) -> torch.tensor:
        ...
        labels = torch.stack(
            (self.real_prompts[self.curr_real_idx], self.fake_prompts[self.curr_fake_idx]))
